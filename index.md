...
- 若該次拍照辨識**沒有**可用的發票 QR，則會改以 §2.3 雲端 AI 辨識出的**商家名稱**（經正規化後的字串）存入同一張 `merchant_category_map` 表，同樣用於下次自動帶入分類。此對應表**不會被傳送至本 App 的伺服器或任何第三方**；它與其他記帳資料一樣存放在本機 SQLite，因此**會隨 §4.1 的匯出檔、§4.3 的 Google 自動備份與換機轉移一併帶走**。可透過「設定 → 清除所有資料」移除
