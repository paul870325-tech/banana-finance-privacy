...
### 1.1 完全本地儲存
本 App 所有由您輸入的資料（包含但不限於：收入、支出、分類、備註、股票代碼、加密貨幣代碼、買入均價、持有數量、存款金額、資產項目、儲蓄目標等），**僅由本 App 儲存在您的裝置本機**（使用 SQLite 資料庫）；因存放於本機 SQLite，這些資料**會隨 §4.1 的匯出檔、§4.3 的 Google 自動備份與換機轉移一併帶走**。

除您主動使用之拍照辨識、廣告與 Premium 訂閱等功能所涉及之第三方服務（另詳 §1.4、§1.8、§2.3、§2.4），以及您啟用 Android 系統層級備份時由**作業系統**執行之 Google 雲端自動備份與換機轉移（另詳 §4.3）外，我們**不會**將上述任何由您輸入的記帳 / 股票 / 資產資料上傳至任何雲端伺服器，也**不會**傳送給開發者或任何第三方。

### 1.2 不蒐集個人識別資訊
本 App **開發者本身不會**蒐集以下任何資訊（下列第 1.4 節說明的 Google AdMob、第 2.4 節說明的 Firebase 服務透過 SDK 自行蒐集者，以及您主動使用 §2.2 意見回饋表單（含您**選擇性上傳**的圖片）或依 §5.3 寄送 Email 與開發者聯繫時、由您自行提供的內容除外）：
...
您可透過「設定 → 清除資料 → 清除快取」或「設定 → 清除資料 → 清除所有本機資料」清除本機日誌。
...
- 若該次拍照辨識**沒有**可用的發票 QR，則會改以 §2.3 雲端 AI 辨識出的**商家名稱**（經正規化後的字串）存入同一張 `merchant_category_map` 表，同樣用於下次自動帶入分類。此對應表**不會被傳送至本 App 的伺服器或任何第三方**；它與其他記帳資料一樣存放在本機 SQLite，因此**會隨 §4.1 的匯出檔、§4.3 的 Google 自動備份與換機轉移一併帶走**。可透過「設定 → 清除資料 → 清除所有本機資料」移除
...
| `url_launcher` | 以系統瀏覽器或對應 App 開啟外部連結（Play 商店與訂閱管理、法務頁面內的連結、§2.2 的「回報辨識問題」表單） | 否（本 App 僅將網址交給系統；連線由外部瀏覽器發出，該連線讓對方取得您的 IP／User-Agent，詳見 §2.2） |
...
### 2.2 意見回饋（選用）
本 App 有**兩個互相獨立**的意見回饋管道，以不同方式開啟，送往**不同的兩份** Google 表單：

**（一）「設定 → 聯絡我們」——App 內建網頁檢視、免登入**

- 此功能會在 App 內建的網頁檢視中開啟外部 Google 表單（forms.gle）。
...
- 送出此表單**不需要登入任何帳號**，也不需要提供 Email（除非您自行填寫聯絡方式）。

**（二）「回報辨識問題」——系統瀏覽器外開、可附截圖、需登入 Google**

- 各 AI 辨識結果頁的「回報辨識問題」，會以**您裝置的系統瀏覽器**（而非 App 內建網頁檢視）開啟**另一份**外部 Google 表單。本 App 僅將網址交給系統，開啟後的一切行為由該瀏覽器與 Google 進行，本 App 無從得知您是否填寫或送出。
- 該表單含「檔案上傳」題，**Google 因此強制要求填答者登入 Google 帳號才能送出**（此為 Google 對含檔案上傳題之表單的一律規定，非本 App 開發者的設定，且無法關閉）。故 Google 端會知悉您的帳號身分，受 [Google 隱私權政策](https://policies.google.com/privacy) 規範。
- 開發者已將該表單設為「**不收集電子郵件地址**」，因此您送出的回覆內容**不含您的 Email**；相對地，開發者也**無法**就您的回報主動回覆您。
- 您可**選擇性**上傳辨識錯誤的截圖。上傳的圖片會存放於**開發者的 Google 雲端硬碟**。
- ⚠ **要上傳什麼完全由您決定**：發票、收據或券商對帳單的截圖可能含店名、消費明細、金額、帳號末碼等資訊。若您不希望提供其中某些內容，請**先自行塗改或裁切後再上傳**。本 App 不會替您處理圖片，也不會自動幫您挑選或上傳任何檔案——所有動作都在瀏覽器中由您親自完成。
- ⚠ 此處的圖片與 §2.3 拍照辨識的圖片**是兩件事**：§2.3 送去辨識的圖片即傳即丟、不被儲存；此處是您主動上傳到表單、**會被保存**（保留期限見 §5.3）。兩者互不相干，您完全可以只回報文字而不附圖。

**兩份表單共通的說明**

- 兩份表單皆由本 App 開發者建立。您送出的內容（含您**若自行填寫**的聯絡方式，以及您**若選擇上傳**的圖片）除由 Google 依其隱私權政策處理外，也會進入開發者綁定的 Google 試算表與 Google 雲端硬碟，**僅用於回覆與追蹤您回報的問題、改善辨識準確度**，不會用於行銷或其他目的；您可依 §5.3 的方式要求刪除。
- 本 App **不會**在開啟任一表單時自動附帶任何您的記帳／股票／資產資料，也不會自動帶入版本號、裝置資訊或辨識結果（表單欄位一律由您自行填寫）。
- 您可選擇不開啟這些頁面或不填寫，皆不影響 App 任何功能。
...
3. 圖片**不會被本服務儲存**（傳輸完成即刻丟棄，僅暫存於記憶體中處理）。股票對帳單的「多筆辨識」模式下，辨識結果會連同各筆交易的**加密憑證**一併回傳；憑證在 App 端僅於該次挑選操作期間暫存於記憶體（離開畫面即消失、不會寫入裝置儲存空間）。當您選擇解鎖其中幾筆時，App 會把對應憑證傳回本服務解密後回傳，圖片與交易內容同樣即傳即丟、不儲存（本服務僅另行保存 §2.4 所述的免費解鎖序號與次數計數）
...
| **Firebase Cloud Functions** | §2.3 所述的圖片內容、拍照記帳一併送出的收支分類名稱清單，以及多筆辨識解鎖時由 App 送回的加密憑證（皆即傳即丟、不儲存） | 將圖片與分類名稱清單轉送 Gemini API 並回傳 OCR 結果；解鎖時於伺服器端解密憑證並回傳該筆交易內容 | asia-east1 區域 |
| **Firebase Firestore** | 匿名 UID + 計數所屬月份 + 當月呼叫次數計數 + 方案別（免費／Premium 訂閱）+ 多筆辨識的「免費解鎖」序號與到期時間（隨機字串，24 小時內有效，不含辨識內容） | 執行每月呼叫次數限額與多筆辨識的免費解鎖額度。**保留期限：自最後一次使用起最多保留約 13 個月**，逾期由排程作業自動刪除。 | asia-east1 區域 |
...
- 惟還原**完成後**，本 App 會照常執行下列已於前文揭露的常規連線，**皆不傳送上述財務紀錄**：
  - 依 §2.4，把備份檔記載的「當月 AI 辨識已用次數」併入您目前匿名帳號的計數（避免以重裝／還原重置免費額度）；傳送內容僅為匿名 UID、月份與次數。這條連線送出的就是上列內容本身，不附帶備份檔的其他資料；日後若有其他欄位需要送出，會在本政策一併揭露。
...
- 您可隨時於「設定 → 清除資料 → 清除所有本機資料」刪除本機所有資料。
- **解除安裝本 App** 會移除您**這台裝置上**所有本機儲存的資料。
- 但請注意：若您已啟用 Google 雲端自動備份（見 **§4.3**），Google Drive 內那份備份**不會**因解除安裝而消失，日後於同一 Google 帳號重新安裝本 App 時**可能被系統自動還原**。若要一併清除雲端那份，請至 **Android 設定 → Google → 備份**刪除本 App 的備份，或先於 App 內執行「設定 → 清除資料 → 清除所有本機資料」再解除安裝。
...
| 資料 | 路徑 | 用途 | 保留期限 |
|------|------|------|---------|
| 每月呼叫次數計數 | `quotas/{anonUid}` | 拍照辨識每月呼叫上限管控，防止濫用（內容為匿名 UID、計數所屬月份、當月已呼叫次數、方案別，以及股票多筆辨識的「免費解鎖」序號與到期時間——隨機字串、24 小時內有效，**不含圖片或辨識內容**） | **自最後一次使用起最多約 13 個月**，逾期由排程作業自動刪除 |
...
- **信件內容請提供**：若您曾訂閱 Premium，請附上**訂單編號**（訂閱有效期間可於本 App 訂閱頁「設定 → 升級訂閱」最下方長按複製；訂閱已結束者請於 Google Play 的訂單紀錄查詢同一編號）——這是我們唯一能將您的申請對應到 §5.2 伺服器端紀錄的資訊
- **處理時程**：自收件起 **30 日內**處理完成並回覆
- **來信的保留與刪除**：您寄送至上述信箱的信件（含寄件人 Email 與信件內容）由開發者於處理完畢後保留至多 **6 個月**供爭議追溯，逾期刪除。本 App 為個人開發，此項清理由開發者**定期人工執行**，並非自動化流程；若您希望提前移除，可來信**要求即時刪除**，我們會優先處理。同樣的保留與刪除方式適用於您依 §2.2 送出的意見回饋表單內容，包含您在「回報辨識問題」表單中**選擇性上傳的圖片**——該圖片存放於開發者的 Google 雲端硬碟，於問題處理完畢後同樣保留至多 6 個月，逾期刪除。

> 註：申請刪除後，您原有的 Premium 訂閱憑證對應紀錄將一併移除，未來無法以該裝置 / 匿名 UID 自動還原 Premium 狀態（仍可透過 Google Play 的「還原購買」流程重新綁定）。

> 刪除處理範圍：附上訂單編號時，我們可由其對應出您的匿名 UID，進而刪除 §5.2 表格所列的全部項目——每月呼叫次數計數、訂閱憑證、配額退還失敗紀錄，以及訂閱狀態稽核紀錄中屬於您的匿名 UID 片段。

> 註：若您**從未訂閱** Premium，伺服器端與您相關的僅有以匿名 UID 為索引的每月呼叫次數計數，以及（若曾發生退還失敗的）配額退還失敗紀錄；這些紀錄**不含任何可識別您個人的資訊**，我們無從比對出哪一筆屬於特定使用者，因此無法針對個別申請執行刪除。該類紀錄會依 §5.2 表列期限（分別為最多約 13 個月與最多 90 天）自動刪除，無須申請。
